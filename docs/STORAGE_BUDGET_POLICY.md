...
- 새 versioned config에는 기존 의미를 유지하는 모드와 예산 분리 모드를 명시한다.
  정확한 schema version/key/범위는 구현 첫 단계에서 contract fixture와 함께 확정한다.
...
- 새 설치 기본 모드와 T/W/F 기본값은 아직 미확정이다. 첫 단계에서 실제 규모·작은 디스크·
  낮은 예산 fixture로 근거를 기록하고 확정한다. 숫자가 미확정인 채 제품 기본값을 구현하지 않는다.
