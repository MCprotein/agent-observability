...
| `private-codex-details` | `false` | `true`, `false` | 이후 Codex notify가 제공하는 cwd, input messages, last assistant message를 authenticated localhost bounded detail queue로 전달하고, 별도 bounded content-free status queue와 mutation-lock 직렬화 writer lane으로 private 저장 |
