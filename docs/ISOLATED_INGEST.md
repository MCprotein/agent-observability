...
**후속 결정 — 2026-09-08:** 사용자는 unsafe/FFI 예외를 거부했고,
[저장 예산 분리 계획](STORAGE_BUDGET_POLICY.md)을 먼저 진행하기로 했다. 아래 격리/VFS
후보는 검토 이력이며 현재 구현 우선안이 아니다. 이 계획을 새 정책의 필수 선행 작업으로
재개하지 않는다. 기존 transaction과 안전성 검증 원칙은 유지한다.
