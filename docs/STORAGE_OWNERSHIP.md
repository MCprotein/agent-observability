...
### 복구 연결의 선행 작업: 기존 render 잠금 유지

현재 catalog 복구는 render 잠금을 만들 수 있는 기존 경로를 사용한다. 별도 사전 검사에서
기존 잠금을 확인하고 해제한 뒤 이 경로를 호출하면, 검사와 실행 사이에 사라진 잠금을
다시 생성할 수 있다. 첫 복구 증분은 생성 없이 기존 잠금을 비차단 획득하고 정확한
descriptor/path identity를 유지하는 별도 permit 경계에 한정한다.

기존 catalog 정리 본문은 private locked helper로 공유하고, 기존 public 복구의 생성 가능
동작은 보존한다. 후속 허가 경로는 기존-only permit을 검사부터 정리까지 유지·재사용해야
