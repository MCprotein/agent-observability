...
| P3–P4 | 수집·보고서·정리 연결, CLI/웹 설정; 선택적 수집 commit 직전 검사 경계와 전체 예약 R 계산 API를 독립 검증·커밋, 실제 검사 구현·작업별 연결 진행 중 | transaction 유지, 명시적 전환, 실제 적용 revision·중단 이유 표시; 기본 수집 경로와 분리 모드 활성화는 아직 변경하지 않음 |
