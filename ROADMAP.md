...
| v1.4.0 | Ready | Local onboarding and documentation usability | One-command private runtime setup opens the first self-contained dashboard, blocked demo ingest cannot report false success, all supported runtime options can be changed through a collision-safe atomic save, setup/demo/config/dashboard process tests pass, README follows run-to-first-value progressive disclosure with renderable Mermaid flows and concise capability/support tables, automatic collection gaps remain explicit, independent code and documentation review clear |
