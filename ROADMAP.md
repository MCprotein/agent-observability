...
| P3–P4 | 수집·보고서·정리 연결, CLI/웹 설정; 첫 증분으로 기존 admission을 보존하는 선택적 수집 commit 직전 검사 경계와 전체 예약 R 계산 연결 구현 중 | transaction 유지, 명시적 전환, 실제 적용 revision·중단 이유 표시; 기본 수집 경로와 분리 모드 활성화는 아직 변경하지 않음 |
