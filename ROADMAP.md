...
| v1.10.0 | In progress | Explainable source context and private detail visibility | Replace ambiguous `unknown` rendering with field-level availability reasons, populate verified project/repository and bounded source-location metadata, and add standalone-only explicit opt-in private request detail storage and inspection; assistant response content remains unavailable unless an official source supplies it | Versioned schema and migration fixtures, default-off and no-team-transmission privacy fixtures, bounded storage/performance tests, Rust/TypeScript parity, Chromium detail disclosure QA, official Codex compatibility evidence, and role-separated independent review required |
