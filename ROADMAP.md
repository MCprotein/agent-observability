...
| v1.9.0 | In progress | Human-friendly local web entrypoints | Make `settings` the primary configuration command with `ui` compatibility, deliver the self-contained dashboard through a capability-separated reloadable `127.0.0.1` server, preserve file artifact compatibility, and add real Chromium loopback QA; commercial team remains TODO | Rust/CLI security tests, TypeScript checks, file-open compatibility smoke, real loopback Chromium desktop/mobile smoke, docs consistency, and role-separated independent review required |
