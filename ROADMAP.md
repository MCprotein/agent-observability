...
이번 안정화 범위와 종료 조건은 아래 네 항목으로 고정한다. 새 API·저장 구조·기능을 추가하지
않고, 이 항목의 실패를 재현하고 해결하는 데 필요한 수정만 수행한다.

| 항목 | 종료 증거 |
| --- | --- |
| 실제 설치 연결 | Codex 설정 충돌 원인 확인, 비소유 설정 보존, 수집기 상태 확인 |
| 실제 데이터 수집 | 동일 runtime에서 새 실제 이벤트가 저장되는 것을 전후 비교로 확인; fixture 결과와 구분 |
| 대시보드 | Chrome 확장 세션의 기존/전용 그룹 탭에서 실제 데이터·필터·상세 표시 확인; 반복 탭 열기 금지 |
| 병합 판단 | 정확한 PR head의 필수 CI와 독립 리뷰; 장시간 검증이 남으면 미완료로 명시 |
