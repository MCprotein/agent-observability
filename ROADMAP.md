...
| v1.9.1 | In progress | Settings status and LaunchAgent recovery hotfix | Refresh Codex integration status without page reload, ignore stale overlapping responses, keep session expiry authoritative, retry one transient startup-unavailable state, and tolerate bounded asynchronous `launchctl bootout` convergence | TypeScript/generated-JS parity, lockfile/README version gates, Chromium retry/focus/heartbeat/session-expiry races, LaunchAgent convergence/fail-closed tests, full Rust/npm validation, live Codex ingest growth, and role-separated independent review required |
