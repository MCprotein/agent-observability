...
| v0.13.0 | Planned | Local-only release candidate | install/config path, bounded collection/flush/storage policy, singleton/crash/full-channel fixtures, adaptive load shedding, capacity/large-log bounds, `cargo run -p xtask -- perf local --profile release --check` evidence for hook latency and CPU/RSS/disk/network budgets, docs and independent review; age retention/archive remain v1.2 |
