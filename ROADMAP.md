...
| v1.9.0 | In progress | Human-friendly local web entrypoints | Make `settings` the primary configuration command with `ui` compatibility, deliver the self-contained dashboard through a capability-separated reloadable runtime-stable `127.0.0.1` origin, preserve saved views and file artifact compatibility, give settings-launched dashboards an independent process lifetime, and add real Chromium loopback QA; commercial team remains TODO | Rust/CLI security and 32 MiB artifact-bound tests, TypeScript checks, file-open compatibility smoke, two-process saved-view smoke, settings-exit lifecycle smoke, real loopback Chromium desktop/mobile smoke, docs consistency, and role-separated independent review required |
