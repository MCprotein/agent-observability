...
배포 검사 진단 보강: CI `34355622898`은 lifecycle/real-Codex 검증 이후 첫 벤치마크
회차에서 실패했다. 완료된 회차가 없고 요약 오류만 남아 최초 원인은 미확정이다.
검사 전용 stderr에 고정 단계·allowlist 오류 코드·수락된 synthetic 요청 수를 남기는
작은 후속 수정으로 원인을 구분했다. 후속 CI `34428226492`에서 1만 건 수락 후
보고서 반영 시간 초과를 확인했다. 저장소 추론의 반복 스캔을 인덱스의 첫 값/다음 값
탐색으로 바꾸고 추론 후보가 없을 때 동일 JSON 재쓰기를 생략했다. 20,001행의 unknown·
중복 후보 회귀 검사와, 유휴 구간을 제외한 release 바이너리 1만 건 재현이 기존 10초
반영 제한을 유지한 채 통과했다. 원문 로그 공개, 성능 기준 완화, 저장 예산 변경은 없으며
정식 장시간 검사 통과 전에는 Released로 전환하지 않는다.
