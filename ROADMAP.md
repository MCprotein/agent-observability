...
- 로드맵 작업은 구체적인 버전 행에 들어간다. 떠 있는 backlog 항목으로 두지 않는다.
...
- major version은 제품의 운영 경계가 바뀌는 큰 단계에만 쓴다. 예를 들어 local-only
  제품화, 중앙 collector 도입, gateway/control-plane 도입처럼 저장 위치, 운영 모드,
  책임 경계가 달라지는 경우다.
...
| v2.x | Planned | Optional internal collector: 팀/프로젝트 단위 집계, 인증, retry-safe ingest를 추가한다. |
| v3.x | Planned | Optional gateway/control plane: 요청 라우팅과 실제 과금 통제까지 다룬다. |
...
| v2.0.0 | Planned | Optional internal collector | auth, schema validation, central trace/metrics/audit stores, retry-safe ingest, redaction-count verification |
| v2.1.0 | Planned | Team aggregation and alerting | team/repo dashboards, cost/error spike alerts, collector failure/retry tests |
| v3.0.0 | Planned | Optional gateway PoC | provider-compatible routing, request attribution, billing reconciliation assumptions, Desktop App setting inheritance checks |
