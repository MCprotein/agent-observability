/* Generated from TypeScript and dashboard-query-v1.schema.json. Do not edit. */
"use strict";
(() => {
  // ui/report/generated/validate-dashboard-query-v1.ts
  ...
  function hasValidSnapshotCounters(value) {
    if (!isRecord(value)) return false;
    if (value.kind === "status" && value.snapshot === void 0) return true;
  ...
  function validateDashboardQueryResponseV1(value) {
  ...
  var pattern42 = new RegExp("^id:sha256:[0-9a-f]{64}$", "u");
  ...
  // ui/report/paged-client.ts
  var DASHBOARD_SCHEMA_VERSION = "agent_observability.dashboard_query.v1";
  var DASHBOARD_REQUEST_BYTES = 8192;
  var DASHBOARD_RESPONSE_BYTES = 1048576;
  var PRIVATE_DETAIL_RESPONSE_BYTES = 65536;
  var DASHBOARD_CURSOR_HISTORY_LIMIT = 16;
  var emptyPage = () => ({
    rows: [],
    nextCursor: null,
    page: 0,
  ...
  var PagedDashboardClient = class {
    transport;
    ...
    draining = false;
    controller;
    privateDetailController;
    wakeTimer;
    traceCursors = [null];
    spanCursors = [null];
    selectedSpanRequestId;
    privateDetailRevision = 0;
    idleWaiters = [];
    constructor(options) {
    ...
    setFilters(filters) {
      ...
      this.selectedSpanRequestId = void 0;
      this.update({
        filters: normalized,
        summary: { state: "idle" },
        traces: emptyPage(),
        ...
        availability: this.stateValue.snapshot === void 0 ? "loading" : availabilityOf(this.stateValue.snapshot),
        reason: void 0
      });
      if (this.stateValue.snapshot === void 0 || this.stateValue.availability === "expired") {
    ...
    clearSelectedDetail() {
      this.selectedSpanRequestId = void 0;
      this.clearPrivateDetail();
      this.update({ selectedSpan: void 0 });
    }
    async loadPrivateDetail() {
      ...
      try {
        ...
        if (revision === this.privateDetailRevision) this.privateDetailController = void 0;
      }
    }
    nextTraces() {
    ...
    async settled() {
      if (!this.draining && this.queue.length === 0 && this.wakeTimer === void 0) return;
    ...
    enqueueBound(purpose, fields, cursor, page, delay = 0) {
      const snapshot = this.stateValue.snapshot;
      if (!snapshot) return;
      ...
      const request = {
        ...
        ...nonCursorFields,
        ...requestCursor === null || requestCursor === void 0 ? {} : { cursor: requestCursor }
      };
    ...
    applyResponse(task, response) {
      ...
      const requestedSnapshotId = task.request.snapshotId;
      if (requestedSnapshotId !== void 0 && response.snapshot.id !== requestedSnapshotId) {
      ...
      this.update({
        ...
        reason: response.scope.coldExcluded ? "cold_excluded" : void 0
      });
      switch (response.kind) {
        ...
        case "traces":
          ...
          break;
        case "spans":
          if (task.request.traceId !== this.stateValue.selectedTraceId) break;
        ...
        case "facets":
          ...
          break;
        case "span":
          if (task.request.spanId === this.selectedSpanRequestId) {
            const detail = response.detail;
            this.update({
              selectedSpan: detail,
              privateDetail: privateDetailEligible(detail) ? { state: "eligible" } : { state: "not_applicable", reason: "This span is not eligible for private local detail." }
            });
          }
          break;
      }
    }
    applyTracePage(response, page) {
      this.update({ traces: { ...pageState(response.rows, response.pagination, page), canGoBack: this.traceCursors.length > 1 } });
    }
    applySpanPage(response, page) {
    ...
    invalidatePendingWork() {
      ...
      this.queue = [];
      if (this.wakeTimer !== void 0) clearTimeout(this.wakeTimer);
      this.wakeTimer = void 0;
      this.clearPrivateDetail();
    }
    clearPrivateDetail() {
      ...
      this.privateDetailController?.abort();
      this.privateDetailController = void 0;
  ...
  function createDashboardTransport(options) {
  ...
  function dashboardQueryEndpoint(location, endpoint) {
    const currentPath = location.pathname.replace(/\/$/, "");
    return new URL(endpoint ?? `${currentPath}/query`, location.href);
  }
  function createPrivateDetailTransport(options) {
    const fetchValue = options.fetch ?? globalThis.fetch;
  ...
  var DashboardTransportError = class extends Error {
  ...
  async function readBoundedResponse(response, limit, label = "Dashboard") {
  ...
  function baseRequest(kind, filters) {
    return { schemaVersion: DASHBOARD_SCHEMA_VERSION, kind, ...Object.keys(filters).length === 0 ? {} : { filters } };
  }
  function pageState(rows, pagination, page) {
    return {
      rows,
  ...
  function availabilityOf(snapshot) {
    return snapshot.state;
  }
  function normalizeFilters(filters) {
    const result = {};
    for (const key of ["repo", "session", "agent", "model"]) {
  ...
  function isProjectedId(value) {
    return /^id:sha256:[a-f0-9]{64}$/.test(value);
  }
  function privateDetailEligible(span) {
    return span.turnId !== void 0 && [
  ...
  function boundedFailureReason(body) {
    try {
      const value = JSON.parse(body);
      return typeof value.code === "string" && /^[a-z0-9_]{1,64}$/.test(value.code) ? `Capture failed (${value.code}).` : "Private detail is temporarily unavailable.";
    } catch {
  ...
  function startPagedDashboard(options) {
    const client = new PagedDashboardClient({
      transport: createDashboardTransport({
        ...options,
        validate: options.validate ?? validateDashboardQueryResponseV1
      }),
  ...
  function bindControls(client) {
    const refresh = document.createElement("button");
    refresh.id = "refresh-dashboard";
    refresh.type = "button";
    refresh.textContent = "Refresh snapshot";
    refresh.addEventListener("click", () => client.refresh());
    required("clear-filters").insertAdjacentElement("afterend", refresh);
    const savedFilterSelect = required("saved-filter");
    const saveFilter = required("save-filter");
    const deleteFilter = required("delete-filter");
    let savedFilters = loadSavedFilters();
    const renderSavedFilterOptions = () => {
    ...
    const resetSavedFilterSelection = () => {
      savedFilterSelect.value = ALL;
      deleteFilter.disabled = true;
    };
    renderSavedFilterOptions();
    required("text-filter").maxLength = 512;
    ...
    required("text-filter").addEventListener("input", () => {
      resetSavedFilterSelection();
      client.setFilters(readFilters());
    });
    required("clear-filters").addEventListener("click", () => {
      for (const id of Object.values(filterIds)) required(id).value = ALL;
      required("text-filter").value = "";
      resetSavedFilterSelection();
      client.setFilters({});
    });
    required("trace-previous").addEventListener("click", () => client.previousTraces());
    required("trace-next").addEventListener("click", () => client.nextTraces());
    required("span-previous").addEventListener("click", () => client.previousSpans());
    required("span-next").addEventListener("click", () => client.nextSpans());
    required("details-close").addEventListener("click", closeDetails);
  ...
  function savedDimensionsFromDashboardFilters(filters) {
    const single = (values) => values?.length === 1 ? values[0] : void 0;
    return {
  ...
  function dashboardFiltersFromSavedDimensions(filters) {
    return {
      ...filters.repo === void 0 ? {} : { repo: [filters.repo] },
      ...filters.session === void 0 ? {} : { session: [filters.session] },
      ...filters.agent === void 0 ? {} : { agent: [filters.agent] },
  ...
  function readFilters(changed) {
    const result = {};
    for (const [key, id] of Object.entries(filterIds)) {
      const select = required(id);
    ...
    const text = required("text-filter").value.trim();
  ...
  function render(state) {
    ...
    renderFacets(state);
    renderKpis(state.summary.state === "complete" ? state.summary.kpis : void 0);
    renderTraces(state);
    renderSpans(state);
    renderDetails(state);
  }
  function renderAvailability(state) {
    ...
    const snapshotDetail = state.snapshot ? `Snapshot ${shortId(state.snapshot.id)} \xB7 generated ${formatTime(state.snapshot.generatedAt)}` : "No query snapshot is currently bound.";
    setText(
      "quality-detail",
      facetDisclosure ? `${snapshotDetail} \xB7 ${facetDisclosure}` : snapshotDetail
    );
    required("clear-filters").disabled = Object.keys(state.filters).length === 0;
    required("refresh-dashboard").disabled = state.availability === "loading" || state.availability === "building";
    required("save-filter").disabled = !isPersistableDimensions(savedDimensionsFromDashboardFilters(state.filters));
  }
  function facetLimitDisclosure(facetsLimited) {
    if (!facetsLimited) return void 0;
  ...
  function renderFacets(state) {
    const values = { repo: [], session: [], agent: [], model: [] };
  ...
  function renderKpis(kpis) {
    ...
    setText("kpi-cost", kpis.costStatus === "estimated" && kpis.estimatedCost !== null ? `Exact estimated ${kpis.currency ?? "USD"} ${Number(kpis.estimatedCost.toPrecision(12))}` : title(kpis.costStatus));
  }
  function renderTraces(state) {
    const list = required("trace-list");
    ...
    required("trace-previous").disabled = !state.traces.canGoBack;
    required("trace-next").disabled = state.traces.nextCursor === null;
  ...
  var activeClient;
  var detailsOpener;
  var pendingDetailsFocusSpanId;
  function renderSpans(state) {
    const body = required("span-table");
    ...
    required("span-previous").disabled = !state.spans.canGoBack;
    required("span-next").disabled = state.spans.nextCursor === null;
    renderTimeline(state.spans.rows.slice(0, state.limits?.timelineRows ?? 120), state);
  }
  function spanRow(span, state) {
    const row = document.createElement("tr");
  ...
  function renderTimeline(spans, state) {
    const list = required("timeline-list");
  ...
  function renderDetails(state) {
    ...
    const panel = required("span-details");
    const body = required("details-body");
    if (!span) {
      panel.classList.remove("open");
      body.replaceChildren(empty("Select a span to inspect safe projected details."));
      if (pendingDetailsFocusSpanId === void 0) detailsOpener = void 0;
      return;
    }
  ...
  function openSpanDetails(spanId, surface, element) {
  ...
  function pageStatus(page, rows, total, hasNext) {
    const totalText = total === void 0 ? "total pending" : `${total.toLocaleString()} total`;
    const continuation = rows === 0 && hasNext ? " \xB7 empty slice, continuation available" : hasNext ? " \xB7 more available" : " \xB7 end";
    return `Page ${page + 1} \xB7 ${rows.toLocaleString()} rows \xB7 ${totalText}${continuation}`;
  }
